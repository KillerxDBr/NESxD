...
void PrintUsage(void) {
    ...
    nob_log(NOB_ERROR, "Usage: ./nob <[b]uild [[w]eb] [[c]lean], [w]eb [python executable], "
                       "bundler [dir], [c]lean>");
}
...
#define USE_FLAGS
    ...
    if (BuildMode) {
        ...
        if (isWeb) {
            ...
            if (nob_file_exists(EMSDK_ENV) != 1) {
                nob_log(NOB_ERROR, "Could not find emsdk env builder \"" EMSDK_ENV
    ...
    if (strcmp(command, "build") == 0 || strcmp(command, "b") == 0) {
        ...
        PrintUsage();
        // nob_log(NOB_INFO, "Unknown command \"%s\", expects: <[b]uild [[w]eb] [[c]lean/cls],
        // bundler [dir], [c]lean/cls>", command);
        nob_return_defer(1);
    }
#endif
...
bool LinkExecutable(bool isWeb) {
    ...
    // gcc -o bin/nesxd.exe build/6502.o build/config.o build/gui.o build/input.o build/main.o
    // build/nob.o build/resource.o build/tinyfiledialogs.o lib/libraylib.a -lgdi32 -lwinmm
    // -lcomdlg32 -lole32
    if (isWeb) {
        ...
        if (nob_needs_rebuild(WASM_OUTPUT, obj.items, obj.count) != 0) {
            ...
            if (nob_file_exists(MEM_BIN_PATH) == 1) {
                ...
                // nob_copy_file(MEM_BIN_PATH, WASM_DIR MEM_BIN_PATH); // Copying dont work, file
                // needs to be bundled
            }
...
bool CompileFiles(bool isWeb) {
    ...
    */
    ...
    // defer:
    //     nob_log(NOB_ERROR, "String Buffer is too small, size: %d, expects: %zu", STR_SIZE,
    //     sb.count); return false;
}
...
bool CompileDependencies(bool isWeb) {
    ...
    /*
    gcc -fdiagnostics-color=always -Ibuild/ -I. -Iinclude -Isrc -Istyles -Iextern -o bin/nesxd.exe
    build/main.o build/6502.o build/input.o build/config.o build/gui.o build/nob.o
    build/tinyfiledialogs.o build/resource.o -Wall -Wextra -Winvalid-pch -std=gnu11 -Og -g3 -ggdb
    -march=native -DKXD_DEBUG -D_UNICODE -DUNICODE -DPLATFORM_DESKTOP -DGRAPHICS_API_OPENGL_33
    ./lib/libraylib.a -lgdi32 -lwinmm -lcomdlg32 -lole32 -DKXD_DEBUG*/
    if (isWeb) {
        EMS(&cmd);
        nob_cmd_append(&cmd, EMCC, "-fdiagnostics-color=never", "-xc", WFLAGS);
    } else {
...
bool WebServer(const char *pyExec) {
    ...
    if (!CloseHandle(p)) {
        nob_log(NOB_ERROR, "Could not close handle to process \"%s\": %s", pyExec,
                nob_win32_error_message(GetLastError()));
        nob_return_defer(false);
    }
...
bool C3Compile(bool isWeb) {
    ...
    const char *tmpOutput = isWeb ? nob_temp_sprintf("%s.o", output) : nob_temp_sprintf("%s.obj",
output);

    Nob_Cmd cmd = { 0 };
    if (nob_needs_rebuild1(tmpOutput, input)) {
        // c3c.exe compile-only src/c3/c3teste.c3 -o build/c3teste --target mingw-x64
--single-module=yes -O1 nob_cmd_append(&cmd, "c3c.exe", "compile-only", input, "-o", output,
"--single-module=yes", C3_OPT, "-D", "NOMAIN"); if (isWeb) nob_cmd_append(&cmd, C3_TARGET_WEB); else
...
#pragma message("[WARN] ParseDependencyFile not implemented for MSVC compiler")
    ...
    if (sv.count > 0 && sv.data[1] == ':')
        nob_sv_chop_by_delim(&sv, ':'); // remove ':' from "C:\\..." on win32
